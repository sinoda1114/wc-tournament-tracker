...
export function GET(): Response {
  ...
  const body = `# WC2026 トーナメントトラッカー（FIFA 非公認・非公式ファンサイト）

> 2026 年の国際的なサッカー大会を「見やすさ」優先で追えるファンサイト。グループリーグ／決勝トーナメント／出場国・スカッド／優勝予想（みんなの予想）／お気に入りを、日本時間（JST）で提供する。結果データは TheSportsDB（無料）。本サイトは FIFA 非公認で、公式ロゴ・エンブレム・公式名称の商標的使用は行わない。
