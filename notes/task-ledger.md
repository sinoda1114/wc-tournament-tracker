...
**🕒 最終更新: 2026-06-13 JST（番人セッション同期: 投票マネタイズ確定→T-51グループ投票窓開放/T-52決勝T課金ゲート起票・T-48自分票のみへ縮小）**
...
- 実行中は **T-48 ADMIN投票リセット（自分票のみ）＋ T-51 グループ投票窓の開放**（同 worktree feat/admin-vote-reset でテスト中）。次のユーザー指定待ちは **T-47 ヒーロー文言**。

### 🟡 その後 — T-12 サイト全体の最終動作チェック（実装出揃い後）
- 全ページ・全言語を通しで触って確認 → 2段ゲート → デプロイ。

### 🟢 着手可（設計確定）
- **T-48** ADMIN 投票リセット機能（**自分票のみ・スコープ縮小2026-06-13**）— `/admin` に isAdmin 限定で**ログイン中の管理者自身の票だけ**をリセット（ステージ別＋自分の全リセット・確認ダイアログ・`voter_id` で限定 DELETE・migration不要）。⚠️「みんなの票を消す」機能は誤爆事故防止のため**持たない**（ユーザー指示）。test→reset→再投票の検証用。**実装完了・worktree feat/admin-vote-reset でテスト中**（PR #57 は旧"みんなリセット"版＝要更新）。（2026-06-13 起票）
- **T-51** グループ投票窓の開放（投票締切ルール改定）— `crowd.ts` の投票締切を「そのステージの初戦KO」→「**次の投票ステージの初戦KO（＝次ステージ開始でロック）**」に変更。グループ戦投票が会期中ずっと open になり、開幕戦で即ロックして "全ステージ締切" の死に体になっていた旧仕様を是正。1ステージ1票ロックは従来どおり。[[voting-monetization-model]] の確定仕様に追従。**実装完了・worktree feat/admin-vote-reset に同梱（テスト用）・テスト中**（tsc/lint/全461テストPASS）。（2026-06-13 起票）
...
⚠️ **T-43 は番号衝突**: 台帳上の T-43（みんなの投票ライフサイクル・未着手）と、実装コミット/PR #43 が名乗った T-43（Wikipediaイベント補完取込・完了）の2件が存在する。台帳では「T-43(投票)」「T-43(Wikipedia)」と区別して記載。以後の新規番号は最大+1（**T-53〜**）を使い、再衝突させない。**使用済**: T-47＝ヒーロー文言変更 / T-50＝勝利チーム表示色の見直し / T-48＝ADMIN投票リセット（自分票のみ） / T-49＝無効ロケールのソフト404のSEO是正 / T-51＝グループ投票窓の開放 / T-52＝決勝T投票の課金ゲート。
