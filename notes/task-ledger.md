...
- **T-57** スマホUI 微調整（タスク T-56／PR #63 本番反映後の実機フィードバック第2弾・2026-06-13 篠田）— ①**右上のレイアウトが微妙** — ヘッダー右上の「インストール(DL)アイコン＋ハンバーガー≡＋ユーザーアバター」の並び/間隔が窮屈・不揃い → 整理 ②**見出しの文字が大きい** — H1（「グループリーグ」等）がスマホで大きすぎ縦を圧迫 → 縮小（`clamp()` で抑制） ③**お気に入りを最左へ** — 「☆のみを表示」（お気に入りフィルタ）は一番左が押しやすい → 配置変更 ④**グループリーグページの位置取りが微妙** — 全体の余白/要素位置を調整。スクショ2枚（トップ／グループリーグ・2026-06-13）あり。ui-feature 領域・worktree→PR。（2026-06-13 起票）
