...
- **#38** 出場選手の在籍クラブ（2026-06-01時点）を全選手分取得 — バックエンドのバッチで取得し選手データに反映、チーム詳細の出場選手欄に所属クラブを表示する。データ源は要調査（TheSportsDB無料APIのlookupplayerにteam情報があるか／レート制限内で全48か国×26人を回せるか・[[squad-data-source]] [[commercial-data-constraints]]の制約に準拠・有料API回避）。重めのタスクなので時間のあるときにバックグラウンドで実行。data-squad 領域
