...
- **T-60** A2HS の文言を「ショートカットを作成」に統一（「インストール」回避）— 篠田希望: 「スマホにインストール」ではなく**「スマホにショートカットを作成」**にしたい。**確認結果（2026-06-13）**: アプリ内ハードコードに「インストール」表示は**無し**。ユーザー表示は `addToHome.label='ホーム画面に追加'`（`AddToHomeScreen.tsx`／i18n ×5言語）で、"install" はコメントとブラウザAPI名（`beforeinstallprompt`/`appinstalled`）のみ。**＝篠田が見た「インストール」は Android Chrome のネイティブ追加プロンプト**（文言はブラウザ管理で変更不可）。対応: ①**トリガー/ボタン label を「ショートカットを作成」へ**（×5言語）②**ネイティブ `prompt()` を呼ばず**、Chrome の「インストール」UI を出さない＝手動「ショートカットを作成」案内に寄せる（T-58③の非強制方針と整合）③ **iOS 手順モーダル内の「ホーム画面に追加」は iOS 実メニュー名なので説明文としては残す**（タップ先の正式名称ゆえ変えると手順が不正確になる）。ui-feature 領域・worktree→PR。（2026-06-13 起票）
