...
- **#36** favicon を MatchFav 用アイコンへ差し替え — 採用候補は `src/app/icon-field-heart-simple-v3.png` / `src/app/apple-icon-field-heart-simple-v3.png`（元データ: `assets/matchfav-favicon-field-heart-simple-v3.svg` / `.png`）。緑のサッカーフィールド＋白い外枠/ゴール/センターサークル＋ゴールド寄りハートの案。既存の `src/app/icon.png`(現156.9K) と `src/app/apple-icon.png` をこの v3 案で置き換え、`src/app/manifest.ts` の参照・サイズ指定が現状どおり機能することを確認する。**背景は透明**（プレビューの黒背景はビューア表示で、実ファイルの角は透過済み＝差し替え時に透過を潰さない）。必要ならブラウザタブ/ホーム画面追加時の見え方も軽く確認する。既存の試作候補（`*-v1`, `*-v2`, `match-card` など）は採用対象外なので、整理するか残すかは担当者判断。ui-feature 領域・#31/#27 と関連（後でやる）
