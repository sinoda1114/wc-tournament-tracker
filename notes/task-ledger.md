...
### 🟠 次 — T-19 i18n B-lite（**実行待ち・GO で夜間自走**）
...
### 🟢 着手可（設計確定）
- **T-43(投票)** みんなの投票のライフサイクル設計＋実装（**方針確定6/12**）。詳細は下。⚠️ T-43 は番号衝突あり（「番号の欠番について」参照）。
...
### ⏳ 待ち（ユーザー操作 / 外部要因）
- **🔴 T-46（差し戻し）** ブラケットR32是正の**本番DB適用が未完**（番人の本番DB操作待ち）。コードは PR #44 でマージ済だが **migration 0012 が本番DBへ未適用**＝本番(matchfav.com)の R32 は依然 実チーム（国旗）を表示。検証2026-06-13: 本番 R32 #73 が `homeTeamId:"kor"` を保持・`agent-sync.md` のボードも `- [ ]` 未処理。**残作業＝番人が本番DBへ 0012 を適用**（idempotent・R32の team_id を NULL 化しスロット表示へ）。台帳の旧「本番適用済」記載は誤りだった。詳細は下。
...
⚠️ **T-43 は番号衝突**: 台帳上の T-43（みんなの投票ライフサイクル・未着手）と、実装コミット/PR #43 が名乗った T-43（Wikipediaイベント補完取込・完了）の2件が存在する。台帳では「T-43(投票)」「T-43(Wikipedia)」と区別して記載。以後の新規番号は最大+1（T-48〜）を使い、再衝突させない（T-47＝ヒーロー文言変更で使用済）。
