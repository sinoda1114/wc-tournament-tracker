...
### ⏳ 待ち（ユーザー操作 / 外部要因）
- **🔴 T-46（差し戻し・診断訂正）** 決勝Tブラケット R32 が本番で実チーム表示のまま（バグ生存）。**番人が本番DBで確認(2026-06-13)**: ① `.env.local` の Turso＝**本番DBで確定**（本番投票が同DBに在る）② `_migrations` に **0012 は executed（2026-06-12 01:46）＝適用済み** ③ それでも本番 R32 が実チームID（#73=kor, #74=civ, #75=jpn, #76=bra…）を保持。**＝旧記載「0012未適用」は誤り。0012は適用済だが R32 が再充填されている**。原因候補＝ingest cron の自動bind（`resolveAndPersistRoundOf32`）がグループ未確定で R32 を埋め戻す疑い。**波及**: T-43 投票がR32を確定扱いし round_of_32 投票を前倒しオープン。**data-squad が調査・修正中**（worktree feat/fix-r32-refill。バグなら根治＋是正 migration 0015、本番適用は番人）。詳細は下。
...
⚠️ **T-43 は番号衝突**: 台帳上の T-43（みんなの投票ライフサイクル・未着手）と、実装コミット/PR #43 が名乗った T-43（Wikipediaイベント補完取込・完了）の2件が存在する。台帳では「T-43(投票)」「T-43(Wikipedia)」と区別して記載。以後の新規番号は最大+1（**T-50〜**）を使い、再衝突させない。**使用済**: T-47＝ヒーロー文言変更 / T-48＝ADMIN投票リセット / T-49＝無効ロケールのソフト404のSEO是正。
