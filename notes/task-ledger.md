...
- ~~T-46 ブラケットR32是正 — 決勝T R32 を確定前は実チーム非表示・グループ枠プレースホルダに是正~~（6/12〜13・PR #44 マージ＋migration 0012 本番適用済）
...
⚠️ **T-43 は番号衝突**: 台帳上の T-43（みんなの投票ライフサイクル・未着手）と、実装コミット/PR #43 が名乗った T-43（Wikipediaイベント補完取込・完了）の2件が存在する。台帳では「T-43(投票)」「T-43(Wikipedia)」と区別して記載。以後の新規番号は最大+1（T-47〜）を使い、再衝突させない。
