...
- **T-56** スマホ画面最適化（ボタン・サイズ・タップ領域）— 現状スマホでボタンや要素のサイズがイマイチ（篠田）。**T-55（ヘッダー面積）は完了済→本タスクはそれ以外の全体**。改善アイディア: ①**タップ領域を 44×44px 以上**に（⭐お気に入り/view切替/言語・TZ・テーマ・DL の小アイコン・WCAG2.2 Target Size）②隣接ボタンの**余白**で誤タップ防止 ③チーム名の**略称過多**（"C…"＝コートジボワール）を緩和（短縮コード CIV＋国旗併記 等）④試合カード（`MatchVersus`/`MatchDayList`）の縦配置・スコア視認性・国旗サイズ ⑤決勝Tブラケットの横スクロール体験（カード幅/スクロールヒント）⑥主操作を**親指圏（下部）**へ（sticky CTA/ボトムバー検討：お気に入り/投票）⑦日付フィルタ/クイックバッジ（`DateFilterBar`）の横はみ出し・タップ性 ⑧順位表（`GroupCard`）の列圧縮で画面内に収める ⑨タイポ/余白を `clamp()` でレスポンシブ化（`globals.css` トークン）。実機 320/375/390 ＋ライト/ダークで確認。ui-feature 領域・worktree→PR。（2026-06-13 起票）
