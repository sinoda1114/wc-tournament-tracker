...
### 🟢 今日中（時間限定）— T-42 自動取込の本番稼働確認
- 今日(6/12)の実試合で、**結果＆イベントが自動で入るか**を初実証。仕組みは実装済（T-2 ingest・T-20b 自動イベント）。**試合がある今日しか実データで確認できない**＝最優先級。詳細は下。
...
- **T-42** 自動取込の本番稼働確認（**今日中・時間限定**）— 仕組みは実装済（T-2 `/api/ingest`＋Vercel Cron 毎時／T-20b 試合イベント自動tier）。**今日(6/12)の実試合で初の本番・実データ稼働確認**。確認点: ①Vercel Cron が毎時 `/api/ingest` を実行してるか（Vercel Logs）②TheSportsDB から今日の試合の結果(スコア/ステータス)が取り込まれサイトに反映されるか ③試合イベント自動tier（得点者/カード）が timeline から入るか（**manual tier 不可侵を維持**）。方法: 試合終了後にサイト/DBで自動反映を確認、必要なら `CRON_SECRET` で `/api/ingest` を手動トリガーして即時確認。リスク: TheSportsDB 無料が WC2026 の fixtures/results/timeline をタイムリーに持つか（無ければ手動tierでカバー＝T-20a）。data-squad/番人 領域。
